...
```
Iteration 1: agent → tool_calls: [get_order("ORD-001")]
             route → "tools"
             tools → {status: "delivered", total: 899.99, ...}
             → back to agent

Iteration 2: agent → tool_calls: [get_customer_history("CUST-42")]
             route → "tools"
             tools → {tier: "gold", refund_count_12mo: 0, ...}
             → back to agent

Iteration 3: agent → tool_calls: [process_refund("ORD-001", 899.99, ...)]
             route → "human_review"  ← HITL fires here
             ⚡ graph pauses
```
...
```
START → [guardrail] ──── blocked ────────────────→ END (polite redirect)
              │
              │ allowed
              ▼
...
```
Main LLM (LangGraph)        DSPy Modules
─────────────────────       ─────────────────────────
Tool selection              Policy assessment
Workflow orchestration      Eligibility decisions
Conversation management     Content generation
Multi-turn reasoning        Classification tasks
```
...
Here's the complete flow for a refund request, from "I want my money back" to "Your refund has been processed". First, a quick look at what happens when the guardrail fires:

```
User: "Write me a poem about my lost package"
                                │
                                ▼
                ┌───────────────────────────────┐
                │ Guardrail Node                 │
                ...
                │ → guardrail_blocked: True      │
                │ → injects canned AIMessage     │
                └───────────────┬───────────────┘
                                │ route → END
                                ▼
          "I'm ShopEasy's customer support agent —
           I can help with orders, refunds, and
           shipping. Is there something order-related
           I can assist with?"
           
          [Agent never invoked. Zero extra LLM cost.]
```

Now the in-scope flow:

```
User: "My laptop stopped working. I want a refund for order ORD-001."
                                │
                                ▼
                ┌───────────────────────────────┐
                │ Guardrail Node                 │
                ...
                ┌───────────────────────────────┐
                │ Agent (Iteration 1)            │
                │ System prompt: "get_order      │
                │ first for refund requests"     │
                │ → tool_calls: [get_order()]   │
                └───────────────┬───────────────┘
                                │ route → tools
                                ▼
                ┌───────────────────────────────┐
                │ MCP Server: get_order()        │
                │ → {status: "delivered",        │
                │    total: 899.99,              │
                │    delivered_date: "Jan 20"}   │
                └───────────────┬───────────────┘
                                │ edge → agent
                                ▼
                ┌───────────────────────────────┐
                │ Agent (Iteration 2)            │
                │ Has order → needs history      │
                │ → tool_calls: [get_history()]  │
                └───────────────┬───────────────┘
                                │ route → tools
                                ▼
                ┌───────────────────────────────┐
                │ MCP Server: get_history()      │
                │ → {tier: "gold",               │
                │    refund_count_12mo: 0}        │
                └───────────────┬───────────────┘
                                │ edge → agent
                ...
                ┌───────────────────────────────┐
                │ Agent (Iteration 3)            │
                │ Has all context → process      │
                │ → tool_calls: [process_refund] │
                │   "process_refund" is in       │
                ...
                └───────────────┬───────────────┘
                    ...
                    invoke() returns immediately
                                │
                                ▼
                API: "I'm ready to refund $899.99.
...
User: "yes"
                ...
                ┌───────────────────────────────┐
                │ Agent (Final)                  │
                │ Sees successful refund         │
                │ → no more tool calls           │
                └───────────────┬───────────────┘
                                │ route → END
                                ▼
                "Your refund of $899.99 has been processed.
                 Refund ID: REF-55123. Expected: 3-5 business days."
```
